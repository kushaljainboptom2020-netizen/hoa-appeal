...
import type { GuideFaqItem } from "@/lib/content/guides/types";
...
export function GuideFaqAccordion({ items, guideTitle }: GuideFaqAccordionProps) {
  const baseId = useId();
  const [openId, setOpenId] = useState<string | null>(items[0]?.id ?? null);

  return (
    <section id="guide-faq" aria-labelledby={`${baseId}-heading`} className="scroll-mt-24">
      <h2 id={`${baseId}-heading`} className="text-xl font-semibold text-white sm:text-2xl">
        Frequently asked questions
      </h2>
      <p className="mt-4 leading-relaxed text-slate-300">
        Answers below relate to themes in “{guideTitle}.” Confirm deadlines in your
        governing documents and with a licensed attorney if your association has
